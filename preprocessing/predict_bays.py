...
def prep_dfP(dfP, dims, filter=False):
    ...
    dfP.range_scale(["lat"], range=(1, dims["vines"]),
        split_cols=["camera", "row"])

    dfP.data = dfP.data.drop(["vineyard", "block", "date", "angle",
        "lon", "vine_l", "vine_r", "focal_length", "exposure_time"], axis=1)
...
def get_data(vineyard, block, dates, holdout_rows=None, reset=False, filter=True):
    data_file = os.path.join(f_org.get_label_path(vineyard, block, dates), "bay_input_data.csv")
...
class Bay_Predictor():
    ...
    def train_model(self, dates, holdout_rows=None, vineyard="crawford-beck",
            block=9):
    ...
    def test_model(self, dates, holdout_rows=None, vineyard="crawford-beck",
            block=9):
...
def arg_parse():
    ...
    ap.add_argument("-d", "--dates", required=True, nargs='+',
        help="date of images to display")
...
if __name__ == "__main__":
    ...
    bp.train_model(dates=date, holdout_rows=list(range(15, 22)),
        vineyard=vineyard, block=block)

    score = bp.test_model(dates=date, holdout_rows=list(range(15)),
        vineyard=vineyard, block=block)
